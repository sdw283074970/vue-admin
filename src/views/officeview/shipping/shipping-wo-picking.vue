  <template>
    <div>
      <h2>Picking List</h2>
      <el-button class="gb-button" :disabled="shipOrder.status != 'New Created'&&shipOrder.status != 'Picking'" @click="pltsTableVisible = true" type="primary">Pick Plts</el-button>
      <el-button class="gb-button" :disabled="shipOrder.status != 'New Created'&&shipOrder.status != 'Picking'" @click="ctnsTableVisible = true" type="primary">Pick Ctns</el-button>
      <el-input
        v-model="search"
        style="width:250px"
        size="large"
            ...
            <template slot-scope="scope">
              <el-dropdown v-if="operationVisible">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item @click.native="putbackHandler(scope.row.id)" v-if="pickVisible">Put back</el-dropdown-item>
      ...
      <el-dialog title="Inventory(Pallets View)"
        :visible.sync="pltsTableVisible"
        width="85%"
        top="5vh"
        :lock-scroll="false">
        <picking-plts @referashPickDetails="referashPickDetails"></picking-plts>
      </el-dialog>
      <el-dialog title="Inventory(Cartons View)"
        :visible.sync="ctnsTableVisible"
        width="85%"
        top="5vh"
        :lock-scroll="false">
        <picking-ctns></picking-ctns>
      </el-dialog>
    </div>
  </template>
